...
const filePath = path.resolve('src/app/(dashboard)/seller-dashboard/messages/page.tsx');
    ...
    <div className="font-outfit home-theme text-[var(--color-on-surface)] selection:bg-[var(--color-primary)]/20 selection:text-[var(--color-primary)]">
      <main className="max-w-[1280px] mx-auto p-4 md:p-8 flex flex-col gap-8">
        <header className="mb-2">
          <h1 className="text-[32px] leading-[1.2] font-semibold text-[var(--color-on-surface)] mb-2">Customer Inquiries</h1>
        ...
        {messages.length === 0 ? (
          <section className="bg-[var(--color-surface-container-lowest)] rounded-[1rem] border border-[var(--color-outline-variant)]/30 card-shadow p-[32px] flex flex-col items-center justify-center min-h-[400px] py-20 text-center relative overflow-hidden transition-all duration-300 hover:shadow-lg group">
            ...
            <h2 className="text-[24px] leading-[1.3] font-semibold text-[var(--color-on-surface)] mb-3 relative z-10">No inquiries yet</h2>
...
// Also remove lucide-react imports if we can easily replace it
content = content.replace(/import { MessageSquare, Send } from "lucide-react";\n/g, '');
...
console.log('Successfully updated messages page.');
