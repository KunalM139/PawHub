...
// 1. Update seller orders page header
const pagePath = path.resolve('src/app/(dashboard)/seller-dashboard/orders/page.tsx');
...
// 2. Update buyer orders page header
const buyerPagePath = path.resolve('src/app/(dashboard)/dashboard/orders/page.tsx');
if (fs.existsSync(buyerPagePath)) {
  let buyerPageContent = fs.readFileSync(buyerPagePath, 'utf8');
  const newBuyerPageReturnBlock = `  return (
    <div className="font-outfit home-theme text-[var(--color-on-surface)] selection:bg-[var(--color-primary)]/20 selection:text-[var(--color-primary)] space-y-8 max-w-[1280px] mx-auto w-full">
      <header className="flex flex-col gap-2">
        <h1 className="text-[32px] md:text-[36px] leading-[1.2] font-semibold text-[var(--color-on-surface)] tracking-tight">My Purchases</h1>
        <p className="text-[18px] leading-[1.6] text-[var(--color-on-surface-variant)] max-w-2xl">
  ...
  buyerPageContent = buyerPageContent.replace(/  return \([\s\S]*\}\n?$/, newBuyerPageReturnBlock + '\n}\n');
  fs.writeFileSync(buyerPagePath, buyerPageContent, 'utf8');
}

// 3. Update OrderList component styling
const orderListPath = path.resolve('src/components/shop/order-list.tsx');
let orderListContent = fs.readFileSync(orderListPath, 'utf8');

// We replace the JSX returned by OrderList.
const listRegex = /  return \([\s\S]*\n  \);\n}/;

const newOrderListBlock = `  return (
    <div className="space-y-8 font-outfit text-[var(--color-on-surface)]">
      ...
      {filteredAndSortedOrders.length === 0 ? (
        <section className="w-full bg-[var(--color-surface-container-lowest)] rounded-2xl shadow-sm border border-[var(--color-outline-variant)]/20 min-h-[500px] flex items-center justify-center py-20 transition-all duration-500 hover:shadow-md card-shadow">
...
orderListContent = orderListContent.replace(listRegex, newOrderListBlock);
fs.writeFileSync(orderListPath, orderListContent, 'utf8');

console.log("Successfully updated orders pages and components.");
