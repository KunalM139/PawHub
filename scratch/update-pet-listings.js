...
const filePath = path.resolve('src/app/(dashboard)/seller-dashboard/pet-listings/page.tsx');
...
const returnBlockRegex = /  return \([\s\S]*\}\n?$/;
content = content.replace(returnBlockRegex, newReturnBlock + '\n}\n');

// Update status colors to fit better
const statusColorsRegex = /const statusColors: Record<string, string> = {[\s\S]*?};/;
const newStatusColors = `const statusColors: Record<string, string> = {
...
content = content.replace(statusColorsRegex, newStatusColors);

// Remove lucide-react and dashboardcard imports
content = content.replace(/import { PawPrint, Plus, Pencil, Trash2 } from "lucide-react";\n/g, '');
content = content.replace(/import { DashboardCard } from "@\/components\/dashboard\/dashboard-card";\n/g, '');
content = content.replace(/import { EmptyState } from "@\/components\/dashboard\/empty-state";\n/g, '');

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully updated pet listings page');
