...
export function ShopGallery() {
  ...
  useEffect(() => {
    ...
    const handleCartUpdated = () => fetchWishlistAndCart();
    window.addEventListener("cart-updated", handleCartUpdated);
    return () => window.removeEventListener("cart-updated", handleCartUpdated);
  }, []);

  async function fetchWishlistAndCart() {
    try {
      ...
      if (wishlistData?.wishlist?.productIds) {
        setWishlist(wishlistData.wishlist.productIds.map((p: any) => p._id || p));
      }
      if (cartData?.cart?.items) {
        setCartItemIds(cartData.cart.items.map((i: any) => i.productId._id || i.productId));
      }
    } catch (e) {}
  }
  ...
  return (
    <div className="space-y-8 font-outfit text-[var(--color-on-surface)]">
      ...
      {/* Filter Bar */}
      <div className="bg-[var(--color-surface-container-lowest)] rounded-2xl border border-[var(--color-outline-variant)]/30 card-shadow p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto flex-grow">
          ...
          <div className="relative w-full md:w-auto shrink-0">
            <select
              ...
              className="w-full md:w-auto appearance-none bg-[var(--color-surface-container-low)] border-none rounded-full py-3.5 pl-5 pr-10 text-[var(--color-on-surface)] font-bold text-[14px] tracking-wide focus:ring-2 focus:ring-[var(--color-primary)] cursor-pointer shadow-inner outline-none"
            >
      ...
      {isLoading ? (
        ...
        <div className="rounded-[24px] border-2 border-dashed border-[var(--color-outline-variant)]/50 bg-[var(--color-surface-container-lowest)] p-16 text-center card-shadow flex flex-col items-center">
